...
import {Alert, Button, Card, Col, Container, Row as BootstrapRow, Table} from 'react-bootstrap';
...
export default function App() {
  ...
  const getAllPossibleCompositions = (data) => {
    ...
    data.forEach((row) => {
      if (row.enemyPlayerClass1 && row.enemyPlayerClass2) {
        const arr = [row.enemyPlayerClass1, row.enemyPlayerClass2].sort((a, b) => a.localeCompare(b));
  ...
  const computeBadges = (data) => {
    let myBadges = []
    const allClasses = ["WARRIOR", "PALADIN", "HUNTER", "SHAMAN", "ROGUE", "DRUID", "PRIEST", "MAGE", "WARLOCK"]
    allClasses.forEach(currentClass => {
      const currentClassMatches = data.filter(row => row.enemyPlayerClass1 === currentClass || row.enemyPlayerClass2 === currentClass || row.enemyPlayerClass3 === currentClass || row.enemyPlayerClass4 === currentClass || row.enemyPlayerClass5 === currentClass)
      const currentClassWinRate = currentClassMatches.filter(row => won(row)).length / currentClassMatches.length
      if (currentClassWinRate > 0.53) {
        myBadges.push(new MyBadge(`Good against ${currentClass.toLowerCase()}`, `Win rate is ${(currentClassWinRate * 100).toFixed(1)}%`, "success"))
      }
      if (currentClassWinRate < 0.47) {
        myBadges.push(new MyBadge(`Bad against ${currentClass.toLowerCase()}`, `Win rate is ${(currentClassWinRate * 100).toFixed(1)}%`, "danger"))
      }
    })
    ...
    myBadges.push(new MyBadge(`Longest match duration ${secondsToHms(longestMatch.endTime - longestMatch.startTime)}`, matchSummary(longestMatch), "primary"))
    myBadges.push(new MyBadge(`Shortest match lasted ${secondsToHms(shortestMatch.endTime - shortestMatch.startTime)}`, matchSummary(shortestMatch), "primary"))
    const meanDur = secondsToHms(mean(data.map(row => row.endTime - row.startTime)))
    const medianDur = secondsToHms(median(data.map(row => row.endTime - row.startTime)))
    myBadges.push(new MyBadge('Match duration trends', `Mean match duration: ${meanDur}, Median match duration: ${medianDur}`, "primary"))
    myBadges.push(new MyBadge(`Longest winning streak`, `${longestSequence(data.map(row => won(row)), true)} victories in a row!`, "success"))
    myBadges.push(new MyBadge(`Longest losing streak`, `${longestSequence(data.map(row => won(row)), false)} defeats in a row :(`, "danger"))

    return myBadges
  }

  const won = row => (row.teamColor && row.winnerColor && row.teamColor === row.winnerColor) || row.diffRating > 0 || row.enemyDiffRating < 0
  ...
  const median = array => array.slice().sort((a, b) => a - b)[Math.floor(array.length / 2)]
  ...
  const matchSummary = row => {
    ...
    const enemies = `${enemy(row.enemyPlayerName1, row.enemyPlayerClass1)} and ${enemy(row.enemyPlayerName2, row.enemyPlayerClass2)}`
  ...
  const enemy = (enemyName, enemyClass) => enemyClass ? (enemyName ? `${enemyName} (${enemyClass})` : enemyClass) : enemyName
...

          {
            !importString ? (
                <Alert key={'notice-trimming'} variant={'primary'}>
                  ...
                  and all matches before season 3 beginning (January, 17th).
                </Alert>
            ) : (
                <div>
                  <br/>
                  ...
                  <br/>
                  <strong>Total win rate: {(totalWins / totalMatches * 100).toFixed(2)}%</strong>
                  <br/>
                  <br/>
                        ...
                        <tr key={item.comp}>
                          ...
                          <td>
                            ...
                            <span className="green">
                              {item.aTotal !== 0 ? (item.aWins / item.aTotal).toFixed(2) : "-"}
                            </span>{" "}
                            /{" "}
                            <span className="red">
                              {item.hTotal !== 0 ? (item.hWins / item.hTotal).toFixed(2) : "-"}
                            </span>
                            )
                          </td>
                        </tr>
                  ...
                  <p><span className="green">Green = Alliance</span> <span className="red">Red = Horde</span></p>
                    ...
                    <BootstrapRow xs={1} sm={2} md={2} lg={3} xl={4} xxl={4} className="g-4">
                      {
                        badges.map(badge => (
                            <Col key={badge.title}>
                              <Card key={badge.title} border={badge.appearance} style={{width: '18rem'}}>
